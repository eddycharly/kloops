...
func GetLabelsFromComment(spc plugins.PluginScmClient, org, repo string, number int) ([]*scm.Label, error) {
...
func DeleteLabelFromComment(spc plugins.PluginScmClient, org, repo string, number int, label string) error {
	comment, err := GetLabelReportComment(spc, org, repo, number)
	if err != nil {
		return err
	}
	...
	if existingLabels.Len() == 0 {
		if comment != nil {
			return spc.DeleteComment(org, repo, number, true, comment.ID)
		}
	...
	if comment != nil && comment.Body != newCommentBody {
		return spc.EditComment(org, repo, number, true, comment.ID, newCommentBody)
	}
	if comment == nil {
...
func AddLabelToComment(spc plugins.PluginScmClient, org, repo string, number int, label string) error {
	comment, err := GetLabelReportComment(spc, org, repo, number)
	if err != nil {
		return err
	}
...
func GetLabelReportComment(spc plugins.PluginScmClient, org, repo string, number int) (*scm.Comment, error) {

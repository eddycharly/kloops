...
// IssueCommentHandler defines the function contract for a scm.IssueCommentHook handler.
type IssueCommentHandler func(PluginRequest, *scm.IssueCommentHook) error

// RegisterIssueCommentHandler registers a plugin's scm.IssueCommentHook handler.
func RegisterIssueCommentHandler(name string, fn IssueCommentHandler) {
	issueCommentHandlers[name] = fn
}

// GetIssueCommentHandler returns a map of plugin names to handlers for the repo.
func GetIssueCommentHandler(plugin string) IssueCommentHandler {
	return issueCommentHandlers[plugin]
}

// PullRequestCommentHandler defines the function contract for a scm.PullRequestCommentHook handler.
type PullRequestCommentHandler func(PluginRequest, *scm.PullRequestCommentHook) error

// RegisterPullRequestCommentHandler registers a plugin's scm.PullRequestCommentHook handler.
func RegisterPullRequestCommentHandler(name string, fn PullRequestCommentHandler) {
	pullRequestCommentHandlers[name] = fn
}

// GetPullRequestCommentHandler returns a map of plugin names to handlers for the repo.
func GetPullRequestCommentHandler(plugin string) PullRequestCommentHandler {
...
// type GenericCommentHandler func(scm.GenericCommentEvent) error

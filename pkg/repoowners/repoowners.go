...
func NewClient(gc git.Client, logger logr.Logger, spc scmProviderClient, mdYAMLEnabled func(org, repo string) bool, skipCollaborators func(org, repo string) bool) *Client {
	return &Client{
		git:               gc,
		spc:               spc,
		logger:            logger.WithName("repoowners"),
...
func (c *Client) LoadRepoAliases(org, repo, base string) (RepoAliases, error) {
	log := c.logger.WithValues("org", org, "repo", repo, "base", base)
	...
	if !ok || entry.sha != sha {
		...
		// TODO
		gitRepo, err := c.git.Clone(org, repo, "", nil)
		if err != nil {
...
func (c *Client) LoadRepoOwners(org, repo, base string) (RepoOwner, error) {
	...
	if c.skipCollaborators(org, repo) {
		log.Info(fmt.Sprintf("Skipping collaborator checks for %s/%s", org, repo))
	...
	if err != nil {
		log.Error(err, "Failed to list collaborators while loading RepoOwners. Skipping collaborator filtering.")
		owners = entry.owners
	} else {
...
func (a RepoAliases) ExpandAlias(alias string) sets.String {
	...
	return a[scmprovider.NormLogin(alias)]
}
...
func loadAliasesFrom(baseDir string, logger logr.Logger) RepoAliases {
	path := filepath.Join(baseDir, aliasesFileName)
	b, err := ioutil.ReadFile(path) // #nosec
	if os.IsNotExist(err) {
		logger.Error(err, fmt.Sprintf("No alias file exists at %q. Using empty alias map.", path))
		return nil
	} else if err != nil {
		logger.Error(err, fmt.Sprintf("Failed to read alias file %q. Using empty alias map.", path))
		return nil
	}
	config := &struct {
	...
	if err := yaml.Unmarshal(b, config); err != nil {
		logger.Error(err, fmt.Sprintf("Failed to unmarshal aliases from %q. Using empty alias map.", path))
		return nil
	}
	...
	logger.Info(fmt.Sprintf("Loaded %d aliases from %q.", len(result), path))
	return result
}

func loadOwnersFrom(baseDir string, mdYaml bool, aliases RepoAliases, dirExcludes sets.String, logger logr.Logger) (*RepoOwners, error) {
	o := &RepoOwners{
		RepoAliases:       aliases,
		baseDir:           baseDir,
		enableMDYAML:      mdYaml,
...
func (o *RepoOwners) walkFunc(path string, info os.FileInfo, err error) error {
	log := o.log.WithValues("path", path)
	if err != nil {
		log.Error(err, "Error while walking OWNERS files.")
		return nil
	}
	...
	if o.enableMDYAML && strings.HasSuffix(filename, ".md") {
		...
		if err := decodeOwnersMdConfig(path, simple); err != nil {
			log.Error(err, "Error decoding OWNERS config from '*.md' file.")
			return nil
		}
		...
		if err != nil {
			log.Error(err, fmt.Sprintf("Unable to find relative path between baseDir: %q and path.", o.baseDir))
			return err
		}
	...
	b, err := ioutil.ReadFile(path) // #nosec
	if err != nil {
		log.Error(err, "Failed to read the OWNERS file.")
		return nil
	}
	...
	if err != nil || simple.Empty() {
		...
		if err != nil {
			log.Error(err, fmt.Sprintf("Failed to unmarshal %s into either Simple or FullConfig.", path))
		} else {
			// it's a FullConfig
			for pattern, cfg := range c.Filters {
				...
				if pattern != ".*" {
					if re, err = regexp.Compile(pattern); err != nil {
						log.Error(err, fmt.Sprintf("Invalid regexp %q.", pattern))
						continue
					}
				}
...
func normLogins(logins []string) sets.String {
	...
	for _, login := range logins {
		normed.Insert(scmprovider.NormLogin(login))
	}
	return normed
}
...
func findOwnersForFile(logger logr.Logger, path string, ownerMap map[string]map[*regexp.Regexp]sets.String) string {
	d := path

	for ; d != baseDirConvention; d = canonicalize(filepath.Dir(d)) {
		relative, err := filepath.Rel(d, path)
		if err != nil {
			logger.WithValues("path", path).Error(err, fmt.Sprintf("Unable to find relative path between %q and path.", d))
			return ""
		}
...
func (o *RepoOwners) entriesForFile(path string, people map[string]map[*regexp.Regexp]sets.String, leafOnly bool) sets.String {
	...
	for {
		relative, err := filepath.Rel(d, path)
		if err != nil {
			o.log.WithValues("path", path).Error(err, fmt.Sprintf("Unable to find relative path between %q and path.", d))
			return nil
		}
